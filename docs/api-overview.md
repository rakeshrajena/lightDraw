...
| **Diagram** | Builders, editor (drag / resize / rotate / bends), **wire flow** + status tint + CAN hops |

### Diagram flow (short)

| API | Purpose |
|-----|---------|
| `Diagram.applyFlow(app, root, opts)` | Dashes / packets; `paths`, `playback`, `statusHighlight`, `statusColors` |
