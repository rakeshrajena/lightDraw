/** Test fixtures mirroring examples/demo-chart-catalog.js */
const SAMPLE_OHLC = [
...
export const NON_INTERACTIVE_CHARTS = new Set(['sparklineChart']);

export function chartTestProps(type: string, w = 200, h = 120): Record<string, unknown> {
  const base = { width: w, height: h, x: 10, y: 10 };
  const series = [12, 28, 18, 42, 36, 24];

  if (FINANCIAL_TYPES.has(type)) return { ...base, data: SAMPLE_OHLC };
